...
def main():
    ...
    file_dir = "hdfs://rc1a-dataproc-m-fqapaj5e40w4851r.mdb.yandexcloud.net:8020/data_fraud/2019-09-21.txt"  # задал явно, потому нужно будет через *.txt?
    ...
    string_columns = ['tx_datetime', 'customer_id', 'terminal_id'] #пока задал явно
    numeric_columns = ['tx_amount'] #пока задал явно
    ...
    write_info(df_info, "info_test_{0}".format(file_name))

    df_validation.write.parquet("hdfs://rc1a-dataproc-m-fqapaj5e40w4851r.mdb.yandexcloud.net:8020/data_fraud/df_validation_{0}.parquet".format(file_name[:-4]))
